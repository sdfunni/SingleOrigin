...
from SingleOrigin.image import getAllPixelDists, line_profile
    ...
    Returns:
        (2-tuple) A 2-tuple containing:
            ...
            * **phase**: *(2D array)* the phase of the sample transmittance,
            in radians
            * **error**: *(1D array)* the error - RMSD of the phase gradients
                compared to the CoM - at each iteration step
    """
    flip = not flip
...
def kmeans_segment(data, n_clusters, window=None, pwr=1, show=True,
    ...
    X_train -= np.min(X_train, axis=scanaxes) - 1e-8
    X_train = X_train**pwr
    X_train /= np.max(X_train, axis=scanaxes)
    ...
    labels = clustering.labels_.reshape(scandims)
