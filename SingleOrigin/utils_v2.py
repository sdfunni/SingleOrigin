...
def fit_gaussian2D(data, p0):
    ...
    p0[4] *= -1
    return params
# x0, y0, sig_maj, sig_ratio, ang, A, I_o

def multi_gauss_ss(p0, x, y, z):
    trunc = p0[-1]
    p0 = np.delete(p0, -1)

    p0 = np.reshape(p0, (int(p0.shape[0]/6), 6))
    p0[:, 3] = p0[:, 2] / p0[:, 3]

    for atom in p0:
        [x0, y0, sig_maj, sig_min, ang, A] = atom
        
        test = np.where(((x > x0 - trunc) & (x < x0 + trunc) & 
                         (y > y0 - trunc) & (y < y0 + trunc)))
        x_ = np.take(x, test)
        y_ = np.take(y, test)
        z_ = np.take(z, test)
