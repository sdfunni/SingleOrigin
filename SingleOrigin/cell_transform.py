...
class UnitCell():
    ...
    at_cols : The 2D atom column positions generated by the project_uc_2d()
        method.
    a_3d : The 3D basis vector matrix.
    a_2d : The 2D basis vector matrix.
    ...
    def __init__(self, cif_path, origin_shift = [0, 0, 0]):
        ...
        self.g = metric_tensor(self.a, self.b, self.c, self.alpha,
                                   self.beta, self.gamma)
        self.g_star = np.linalg.inv(self.g)

    def transform_basis(self, za, a1_, a2_):
        ...
        alpha_t=np.array([za, a1_, a2_])
        
        za_mag = bond_length([0,0,0], alpha_t[0], self.g)
        a1_mag = bond_length([0,0,0], alpha_t[1], self.g)
        a2_mag = bond_length([0,0,0], alpha_t[2], self.g)
        ...
        print('transformed lattice parameters:',
              np.around(za_mag, decimals=4), 
              np.around(a1_mag, decimals=4), 
              np.around(a2_mag, decimals=4), 
              np.around(alpha_, decimals=4), 
              np.around(beta_, decimals=4), 
              np.around(gamma_, decimals=4))
        g_ = metric_tensor(za_mag, a1_mag, a2_mag, alpha_, beta_, gamma_)
        g_star_=np.linalg.inv(g_)
        a_3d = np.array([za_mag, a1_mag, a2_mag]) * np.identity(3)
    ...
    def plot_unit_cell(self, label='elem'):
        ...
        ColList = np.sort(pd.unique(self.at_cols[label])).tolist()
        cmap = plt.get_cmap('RdYlGn')
        c = {ColList[x] : np.array([cmap(x/(len(ColList)-1))],
                                        ndmin=2) for x in range(len(ColList))}

        for site in ColList:
            axs.scatter(self.at_cols.loc[self.at_cols[label] == site
                                         ].loc[:, 'x'],
                        self.at_cols.loc[self.at_cols[label] == site
                                         ].loc[:, 'y'],
                        c=c[site], vmin = 0, vmax = 1, s=300)

        for ind in self.at_cols.index:
            axs.annotate(rf'${ self.at_cols.at[ind, label] }$',
