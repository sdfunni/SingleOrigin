...
class UnitCell():
    ...
    at_cols : The 2D atom column positions generated by the 'project_uc_2d()'
        method.
    a_3d_real : The 3D direct structure matrix (i.e. transformation matrix
        from fractional to real cartesian coordinates in Angstroms).
    a_2d_real : The 2D projected direct structure matrix (i.e. transformation
                ...
                for oper in symm_eq_pos:
                    ...
                    while np.allclose(xyz0, xyz_, atol=0.1) is False:
                        ...
                        [new_pos.at['u'], new_pos.at['v'], new_pos.at['w']] = [
                            *xyz_]
                        atoms = pd.concat([atoms, new_pos.to_frame().T],
                                          ignore_index=True)
    ...
    def project_uc_2d(self, proj_axis=0, ignore_elements=[],
        ...
        (_, index, counts) = np.unique(np.array(list(zip(x, y, elem))),
                                       ...
                                       return_counts=True,
                                       axis=0)
        weights = pd.DataFrame(list(zip(index, counts))
