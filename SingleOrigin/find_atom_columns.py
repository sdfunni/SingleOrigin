    ...
    def plot_disp_vects(
        ...
        if sites_to_plot == 'all':
            ...
            if len(sites_not_found) >= 1:
                print(f'!!! Note: Specified sites not found for plotting: \n',
                      f'{sites_not_found} \n')
...
class AtomicColumnLattice:
    ...
    def fit_atom_columns(
        ...
        ***Other peak finding options to be implimented in the future.
        ...
        peak"""
        fit_masks_to_peaks = map_coordinates(
            fit_masks,
            np.flipud(xy_peak.T),
            order=0
        ).astype(int)
        ...
        """Throw out masks which do not correspond to at least one reference
            lattice point"""
        fit_masks = np.where(
            np.isin(fit_masks, LoG_masks_used),
            fit_masks,
            0
        )
        ...
        group_sizes, counts = np.unique(
            [match[1].shape[0] for match in peak_groupings],
            return_counts=True
        )
        ...
        args_packed = [
            [self.image[group_slices[counter][0],
                        group_slices[counter][1]
                        ],
             fit_masks[group_slices[counter][0],
                       group_slices[counter][1]
                       ],
             fit_masks_to_peaks[inds_],
             [group_slices[counter][1].start,
              group_slices[counter][0].start
              ],
             xy_peak[inds_, :].reshape((-1, 2)),
             at_cols_inds[inds_],
             group_mask_num]
            for counter, [group_mask_num, inds_]
        ...
        print(
            f'Done. Fitting time: {int(t_elapse // 60)} min '
        ...
        for i, key in enumerate(vpcfs_to_plot):
            ...
            if plot_equ_ellip:
                ...
                elips = [Ellipse(
                    ...
                    width=peak.sig_maj * ellip_scale_factor,
                    height=peak.sig_min * ellip_scale_factor,
                    angle=-peak.theta,
