...
def LoG_2d(x, y, x0, y0, sig, sig_r, ang, A=1, I_o=0):
    """Sample a 2D, ellpitical Gaussian function.
    ...
    # ang = np.radians(-ang) #negative due to inverted y axis in python
    # du_dx = -((np.cos(ang)**2*(x-x0) 
    #           + np.sin(ang)*np.cos(ang)*(y-y0))/sig_1**2 
    #          + (np.sin(ang)**2*(x-x0) 
    #             - np.sin(ang)*np.cos(ang)*(y-y0))/sig_2**2)
    # du_dy = -((np.sin(ang)*np.cos(ang)*(x-x0) 
    #           + np.sin(ang)**2*(y-y0))/sig_1**2 
    #          + (-np.sin(ang)*np.cos(ang)*(x-x0) 
    #             + np.cos(ang)**2*(y-y0))/sig_2**2)
...
def fit_gaussian2D(data, p0, masks=None, method='BFGS', use_LoG_fitting=False):
